...
$$ d^2 = \text{||}{(\bar{p}_0 - \bar{q}_0) + (\bar{v}_p - \bar{v}_q) \cdot t \text{||}}^2 = \text{||}{\bar{p}_{rel} + \bar{v}_{rel} \cdot t \text{||}}^2.$$

This expression reminds the case from previous paragraph. 
The derivative

$$({\text{||} \bar{p}_{rel} + \bar{v}_{rel} \cdot t \text{||}}^2)' = 2 \cdot ({\text{||} \bar{v}_{rel} \text{||} }^2 \cdot t + \langle \bar{p}_{rel}, \bar{v}_{rel} \rangle) $$

is equal zero when 

$$t = -\frac{\langle \bar{p}_{rel}, \bar{v}_{rel} \rangle}{{\text{||} \bar{v}_{rel} \text{||}}^2}$$
